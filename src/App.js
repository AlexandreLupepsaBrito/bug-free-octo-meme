...
function App() {
  ...
  return (
    <div>
      ...
      <label> Responsavel Tarefa: </label>
      <input
        type="text"
        placeholder="Nome"
        value={autor}
  ...
  {post.map((value) => (
    <li key={value.id}>
      <div><strong>ID: {value.id}</strong></div>
      <div><span>Título: {value.titulo}</span></div>
      <div><span>Autor: {value.autor}</span></div>
